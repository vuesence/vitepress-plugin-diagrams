...
function extractDiagramsFromMarkdown(md) {
  ...
  while (match = codeBlockRegex.exec(md)) {
    ...
    if (!SUPPORTED_DIAGRAM_TYPES.includes(type)) continue;
    const meta = match[2];
    const content = match[3].replace(/\r\n/g, "\n");
    const idMatch = /id=([\w-]+)/.exec(meta);
    blocks.push({ type, content, id: idMatch ? idMatch[1] : void 0 });
  }
  return blocks;
}
...
function clean(options) {
  ...
  for (const mdFile of mdFiles) {
    const md = fs2.readFileSync(mdFile, "utf-8");
...
function checkMissing(options) {
  ...
  const diagramFiles = new Set(fs3.readdirSync(diagramsDir).filter((f) => f.endsWith(".svg")));
  const missing = [];
  const mdFiles = getMarkdownFilesFromDir(docsRoot);
  for (const mdFile of mdFiles) {
    const md = fs3.readFileSync(mdFile, "utf-8");
    for (const { type, content, id } of extractDiagramsFromMarkdown(md)) {
      const filename = generateUniqueFilename(type, content, id);
