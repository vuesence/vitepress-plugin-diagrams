...
var require_punycode = __commonJS({
  "node_modules/.pnpm/punycode.js@2.3.1/node_modules/punycode.js/punycode.js"(exports2, module2) {
    "use strict";
...
// src/utils.ts
function extractDiagramMetadata(tokens, idx) {
  const nextToken = tokens[idx + 1];
...
function generateUniqueFilename(diagramType, diagramContent, diagramId) {
...
function getMarkdownFilesFromDir(docsDir) {
  let results = [];
  const list2 = fs.readdirSync(docsDir);
...
function extractDiagramsMetadataFromMarkdown(filepath) {
  const diagrams = [];
...
function getAllDiagramsHashes(docsRoot) {
  const dir = docsRoot || "docs";
  ...
  for (const mdFile of mdFiles) {
    for (const { type, content, id, caption } of extractDiagramsMetadataFromMarkdown(mdFile)) {
      const filename = generateUniqueFilename(type, content, id);
      hashes.add(filename);
    }
  }
  return hashes;
...
function clean(options) {
  ...
  for (const f of unused) {
    console.log("  " + f);
    if (shouldDelete) {
      fs2.unlinkSync(path2.join(diagramsDir, f));
      console.log("    deleted");
    }
  }
  if (!shouldDelete) {
...
function checkMissing(options) {
  const diagramsDir = resolveDiagramBaseDir();
  if (!fs3.existsSync(diagramsDir)) {
    throw new Error(`No diagrams directory found: ${diagramsDir}`);
  }
  const realDiagrams = new Set(fs3.readdirSync(diagramsDir).filter((f) => f.endsWith(".svg")));
...
program2.name("diagrams").description("CLI tool for managing diagram files").version("1.0.0");
program2.command("clean").description("Clean up unused diagram SVGs").option("--delete", "Actually remove unused SVGs").option("--docs <path>", "Root directory of markdown files", "docs").action((opts) => {
  clean(opts);
});
program2.command("check-missing").description("Find diagrams referenced in markdown but missing in filesystem").option("--docs <path>", "Root directory of markdown files", "docs").action((opts) => {
  checkMissing(opts);
});
program2.parse();
