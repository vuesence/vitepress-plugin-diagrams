...
Ви можете імпортувати визначення діаграм із зовнішніх файлів за допомогою синтаксису `@file:`. Це корисно для:

- Керування складними діаграмами у спеціалізованих редакторах (наприклад, BPMN-редактори, PlantUML IDE)
- Повторного використання визначень діаграм на кількох сторінках документації
- Збереження markdown-файлів чистими та зосередженими на контенті
...
```
<!-- diagram id="1" caption: "Бізнес-процес" -->
````

Шлях вирішується **відносно markdown-файлу**, що містить імпорт.

### Підтримувані типи шляхів

- **Відносні шляхи**: `@file:./diagrams/test.bpmn` або `@file:../shared/test.mmd`
- **Абсолютні шляхи**: `@file:/absolute/path/to/diagram.puml`

### Опції конфігурації

Керуйте поведінкою імпорту файлів за допомогою цих опцій плагіна:

```ts
configureDiagramsPlugin(md, {
  // Увімкнути/вимкнути імпорт файлів (за замовчуванням: true)
  enableFileImports: true,
  
  // Обмежити імпорт певними директоріями для безпеки (опціонально)
  allowedImportDirs: [
    './docs/diagrams',
    './shared/diagrams'
  ],
...
Коли вказано `allowedImportDirs`, можна імпортувати лише файли в межах цих директорій. Це запобігає випадковому або зловмисному доступу до конфіденційних файлів поза межами структури документації.

### Обробка помилок

Якщо файл не можна прочитати (відсутній, відмова в доступі тощо), замість діаграми буде відображено повідомлення про помилку:
...
| `enableFileImports` | `boolean` | `true` | Увімкнути/вимкнути синтаксис імпорту файлів (@file:) |
| `allowedImportDirs` | `string[]` | `undefined` | Обмежити імпорт файлів певними директоріями (безпека) |
