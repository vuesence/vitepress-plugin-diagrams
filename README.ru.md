...
Вы можете импортировать определения диаграмм из внешних файлов с использованием синтаксиса `@file:`. Это полезно для:

- Управления сложными диаграммами в специализированных редакторах (например, BPMN-редакторы, PlantUML IDE)
- Повторного использования определений диаграмм на нескольких страницах документации
- Сохранения markdown-файлов чистыми и сфокусированными на контенте
...
```
<!-- diagram id="1" caption: "Бизнес-процесс" -->
````

Путь разрешается **относительно markdown-файла**, содержащего импорт.

### Поддерживаемые типы путей

- **Относительные пути**: `@file:./diagrams/test.bpmn` или `@file:../shared/test.mmd`
- **Абсолютные пути**: `@file:/absolute/path/to/diagram.puml`

### Опции конфигурации

Управляйте поведением импорта файлов с помощью этих опций плагина:

```ts
configureDiagramsPlugin(md, {
  // Включить/отключить импорт файлов (по умолчанию: true)
  enableFileImports: true,
  
  // Ограничить импорт определёнными директориями для безопасности (опционально)
  allowedImportDirs: [
    './docs/diagrams',
    './shared/diagrams'
  ],
...
Когда указан `allowedImportDirs`, можно импортировать только файлы в пределах этих директорий. Это предотвращает случайный или злонамеренный доступ к конфиденциальным файлам за пределами структуры документации.

### Обработка ошибок

Если файл не может быть прочитан (отсутствует, отказ в доступе и т.д.), вместо диаграммы будет отображено сообщение об ошибке:
...
| `enableFileImports` | `boolean` | `true` | Включить/отключить синтаксис импорта файлов (@file:) |
| `allowedImportDirs` | `string[]` | `undefined` | Ограничить импорт файлов определёнными директориями (безопасность) |
